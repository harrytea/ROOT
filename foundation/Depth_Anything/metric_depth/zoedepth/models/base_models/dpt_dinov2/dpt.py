...
class DPT_DINOv2(nn.Module):
    def __init__(self, encoder='vitl', features=256, use_bn=False, out_channels=[256, 512, 1024, 1024], use_clstoken=False):
        ...
        depth_anything_dir = osp.dirname(osp.dirname(osp.dirname(osp.dirname(osp.dirname(osp.dirname(current_file))))))
        dinov2_path = osp.join(depth_anything_dir, "torchhub", "facebookresearch_dinov2_main")
        self.pretrained = torch.hub.load(dinov2_path, 'dinov2_{:}14'.format(encoder), source='local', pretrained=False)
        
        # self.pretrained = torch.hub.load('/stevezxzhou-cfs/nlp/code/spatialVLM/Depth_Anything/torchhub/facebookresearch_dinov2_main', 'dinov2_{:}14'.format(encoder), source='local', pretrained=False)
